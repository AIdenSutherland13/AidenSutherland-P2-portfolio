...
fit_year <- lm(prob ~ sex, data = sex_death_prob)
```
...
summary(fit_year)
```
The p-value for age is extremely low, indicating that the relationship between age and death probability is statistically significant.
The p-value for sex is not statistically significant, meaning there is no difference in death probability between men and women.
